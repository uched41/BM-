...
*/	
uint32_t set_reminder(user_id_t user_id, uint8_t* data, uint8_t len){
	...
	if (rc == FDS_SUCCESS){
		...
		memcpy(&user_data, record.p_data, sizeof(user_data_t));
                ...
                uint8_t rem_no = (user_data.last_set + 1)%MAX_REMINDERS;
		...
		user_data.reminder[rem_no].is_set = true;
                user_data.last_set = rem_no;
                print_userdata(&user_data);
...
void print_userdata(user_data_t* user){
  NRF_LOG_HEXDUMP_INFO(user, sizeof(user_data_t));
  return;
  for(uint8_t i=0; i<4; i++){
    NRF_LOG_INFO("Rem_No: %u, is_set: %u, len: %u", i, user->reminder[i].is_set, user->reminder[i].len);
   }
}
