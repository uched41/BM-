    ...
    if (p_evt_write->handle == p_cus->custom_value_handles.value_handle)
    {
        // Put specific task here. 
			...
			switch(cmd){
				...
				case 2:{			// Set reminder
                                        addr_ble_t addr;
                                        memcpy(addr.addr, p_evt_write->data+1, 6);
                                        uint32_t u_id = generate_userid(&addr);
					uint8_t len = p_evt_write->data[7];
										
					uint32_t ans = set_reminder(u_id, p_evt_write->data+8, len);
